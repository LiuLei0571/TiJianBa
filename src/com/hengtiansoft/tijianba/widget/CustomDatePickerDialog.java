...
public class CustomDatePickerDialog extends DatePickerDialog {
  ...
  public CustomDatePickerDialog(Context context, OnDateSetListener callBack, int year, int monthOfYear, int dayOfMonth) {
    ...
    this.setTitle(year + "年" + monthOfYear + "月" );
    this.year = year;
  ...
  private void setMaxDate(DatePicker dp) {
    dp.init(year, monthOfYear, 0, new OnDateChangedListener() {
      @Override
      public void onDateChanged(DatePicker view, int year, int monthOfYear, int dayOfMonth) {

        if (isDateAfter(view)) {
          Calendar mCalendar = Calendar.getInstance();
          view.init(mCalendar.get(Calendar.YEAR), mCalendar.get(Calendar.MONTH), 0,
              this);
        }
      }
      ...
      private boolean isDateAfter(DatePicker tempView) {
        ...
        tempCalendar.set(tempView.getYear(), tempView.getMonth(), 0, 0, 0, 0);

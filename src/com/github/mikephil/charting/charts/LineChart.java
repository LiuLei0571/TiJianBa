...
public class LineChart extends BarLineChartBase<LineData> {
  ...
  protected void init() {
    ...
    mCirclePaintInner.setColor(Color.rgb(96, 191, 188));
  ...
  protected void drawHighlights() {
    ...
    for (int i = 0; i < mIndicesToHightlight.length; i++) {
      ...
      LineDataSet set = mData.getDataSetByIndex(mIndicesToHightlight[i].getDataSetIndex());
    ...
    for (int i = 0; i < mData.getDataSetCount(); i++) {
      ...
      if (dataSet.isDrawCubicEnabled()) {
        ...
        if (points.size() > 1) {
          for (int j = 0; j < points.size() * mPhaseX; j++) {
            ...
            if (j == 0) {
              ...
              CPoint prev = points.get(j - 1);
              spline.cubicTo(prev.x + prev.dx, (prev.y + prev.dy) * mPhaseY, point.x - point.dx, (point.y - point.dy)
        ...
        if (dataSet.isDrawFilledEnabled()) {
          ...
          float fillMin = mFillFormatter.getFillLinePosition(dataSet, mData, mYChartMax, mYChartMin);
        ...
        if (dataSet.getColors() == null || dataSet.getColors().size() > 1) {
          ...
          for (int j = 0; j < (valuePoints.length - 2) * mPhaseX; j += 2) {
            ...
            // bounds
            if (j != 0 && isOffContentLeft(valuePoints[j - 1]) && isOffContentTop(valuePoints[j + 1])
        ...
        if (dataSet.isDrawFilledEnabled() && entries.size() > 0) {
          ...
          Path filled = generateFilledPath(entries,
              mFillFormatter.getFillLinePosition(dataSet, mData, mYChartMax, mYChartMin));
      ...
      for (int i = 0; i < mData.getDataSetCount(); i++) {
        ...
        for (int j = 0; j < positions.length * mPhaseX; j += 2) {
          ...
          if (mDrawUnitInChart) {
            ...
            mDrawCanvas.drawText(mValueFormatter.getFormattedValue(val) + mUnit, positions[j], positions[j + 1]
                - valOffset, mValuePaint);
          } else {

            mDrawCanvas.drawText(mValueFormatter.getFormattedValue(val), positions[j], positions[j + 1] - valOffset,
                mValuePaint);
          }
        }
      }

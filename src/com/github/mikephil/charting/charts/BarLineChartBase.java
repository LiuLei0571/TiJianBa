...
public abstract class BarLineChartBase<T extends BarLineScatterCandleData<? extends BarLineScatterCandleRadarDataSet<? extends Entry>>>
  ...
  /**
   * if set to true, the highlight indicator (lines for linechart, dark bar for
   * barchart) will be drawn upon selecting values.
   */
  ...
  protected void init() {
    ...
    mGridPaint.setColor(Color.rgb(217, 226, 227));
    mGridPaint.setStrokeWidth(mGridWidth);
    mGridPaint.setStyle(Style.STROKE);

    mBorderPaint = new Paint();
    mBorderPaint.setColor(Color.rgb(217, 226, 227));
    ...
    mGridBackgroundPaint.setColor(Color.rgb(233, 247, 247)); // light
    // grey
  }
  ...
  /**
   * does all necessary preparations, needed when data is changed or flags that
   * effect the data are changed
   */
  @Override
  public void prepare() {
  ...
  protected void calculateOffsets() {
    ...
    if (mDrawLegend && mLegend != null && mLegend.getPosition() != LegendPosition.NONE) {
      ...
      if (mLegend.getPosition() == LegendPosition.RIGHT_OF_CHART
        ...
        legendRight = mLegend.getMaximumEntryLength(mLegendLabelPaint) + mLegend.getFormSize()
            + mLegend.getFormToTextSpace() + spacing;
    ...
    float ylabelwidth = Utils.calcTextWidth(mYLabelPaint, label + mUnit + (mYChartMin < 0 ? "----" : "+++")); // offsets
    ...
    float min = Utils.convertDpToPixel(16f);
   ...
   * relevant when drawing into the chart. It can be used to refresh the legend.
   */
  ...
  protected void calcModulus() {
    ...
    mXLabels.mXAxisLabelModulus = (int) Math.ceil((mData.getXValCount() * mXLabels.mLabelWidth)
  ...
  protected void prepareXLabels() {
    ...
    int max = (int) Math.round(mData.getXValAverageLength() + mXLabels.getSpaceBetweenLabels());
  ...
  private void drawXLabels() {
    ...
    float yoffset = Utils.convertDpToPixel(6f);
    ...
    if (mXLabels.getPosition() == XLabelPosition.TOP) {
      ...
      drawXLabels(getHeight() - mOffsetBottom + mXLabels.mLabelHeight + yoffset * 1f);
   ...
   */
  private void drawYLabels(float yOffset) {
    ...
    for (int i = 0; i < mBorderPositions.length; i++) {
      ...
      switch (mBorderPositions[i]) {
        case LEFT:
          ...
          break;
        case RIGHT:
          mDrawCanvas.drawLine(getWidth() - mOffsetRight, mOffsetTop, getWidth() - mOffsetRight, getHeight()
        ...
        case TOP:
          ...
          break;
        case BOTTOM:
          mDrawCanvas.drawLine(mOffsetLeft, getHeight() - mOffsetBottom, getWidth() - mOffsetRight, getHeight()
              - mOffsetBottom, mBorderPaint);
          break;
      }
    }
  ...
  protected void drawGridBackground() {
    ...
    Rect gridBackground = new Rect((int) mOffsetLeft + 1, (int) mOffsetTop + 1, getWidth() - (int) mOffsetRight,
  ...
  protected void drawHorizontalGrid() {
    ...
    for (int i = 0; i < mYLabels.mEntryCount; i++) {
      ...
      mDrawCanvas.drawLine(mOffsetLeft, position[1], getWidth() - mOffsetRight, position[1], mGridPaint);
    }
  }
  ...
  /**
   * returns true if the specified point (x-axis) exceeds the limits of what is
  ...
  /**
   * returns true if the specified point (y-axis) exceeds the limits of what is
   * visible on the top
   * 
   * @param v
   * @return
  ...
  /**
   * Zooms in or out by the given scale factor. x and y are the coordinates (in
   * pixels) of the zoom center.
   * 
   * @param scaleX
  ...
  /**
   * If this is set to true, the y-axis is inverted which means that low values
   * are on top of the chart, high values on bottom.
   * 
   * @param enabled
   */
  ...
  /**
   * Centers the viewport around the specified x-index and the specified y-value
   * in the chart. Centering the viewport outside the bounds of the chart is not
   * possible. Makes most sense in combination with the setScaleMinima(...)
   * method. First set the scale minima, then center the viewport. SHOULD BE
   * CALLED AFTER setting data for the chart.
   * 
   * @param xIndex
  ...
  public synchronized void centerViewPort(final int xIndex, final float yVal) {
    ...
    Log.i(LOG_TAG, "indices: " + indicesInView + ", vals: " + valsInView);
  ...
  /**
   * Sets the effective range of y-values the chart can display. If this is set,
   * the y-range is fixed and cannot be changed. This means, no recalculation of
   * the bounds of the chart concerning the y-axis will be done when adding new
   * data. To disable this, provide Float.NaN as a parameter or call
   * resetYRange();
   * 
   * @param minY
   * @param maxY
   * @param invalidate
   *          if set to true, the chart will redraw itself after calling this
   *          method
   */
  ...
  /**
   * if this returns true, the chart has a fixed range on the y-axis that is not
   * dependant on the actual data in the chart
   * 
   * @return
   */
  ...
  /**
   * If set to true, the highlight indicators (cross of two lines for LineChart
   * and ScatterChart, dark bar overlay for BarChart) that give visual
   * indication that an Entry has been selected will be drawn upon selecting
   * values. This does not depend on the MarkerView. Default: true
   * 
   * @param enabled
   */
  ...
  /**
   * Set this to true to enable dragging (moving the chart with the finger) for
   * the chart (this does not effect scaling).
   * 
   * @param enabled
   */
  ...
  /**
   * Set this to true to enable scaling (zooming in and out by gesture) for the
   * chart (this does not effect dragging).
   * 
   * @param enabled
   */
  ...
  /**
   * Returns true if scaling (zooming in and out by gesture) is enabled for the
   * chart, false if not.
   * 
   * @return
   */
  ...
  /**
   * Set this to true to enable zooming in by double-tap on the chart. Default:
   * enabled
   * 
   * @param enabled
   */
  ...
  /**
   * Transforms the given chart values into pixels. This is the opposite method
   * to getValuesByTouchPoint(...).
   * 
   * @param x
   * @param y
   * @return
  ...
  /**
   * returns the y-value at the given touch position (must not necessarily be a
   * value contained in one of the datasets)
   * 
   * @param x
   * @param y
   * @return
  ...
  /**
   * if set to true, both x and y axis can be scaled with 2 fingers, if false, x
   * and y axis can be scaled separately. default: false
   * 
   * @param enabled
   */

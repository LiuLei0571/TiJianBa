 ...
 */
public abstract class Chart<T extends ChartData<? extends DataSet<? extends Entry>>> extends View implements
    AnimatorUpdateListener, ChartInterface {
  ...
  /**
   * string that is drawn next to the values in the chart, indicating their unit
   */
  ...
  /**
   * flag that indicates if the default formatter should be used or if a custom
   * one is set
   */
  ...
  /**
   * object that holds all data that was originally set for the chart, before it
   * was modified or any filtering algorithms had been applied
   */
  ...
  /**
   * text that is displayed when the chart is empty that describes why the chart
   * is empty
   */
  ...
  protected void init() {
    ...
    mDescPaint.setColor(Color.rgb(119, 119, 119));
  ...
  /**
   * Returns true if the chart is empty (meaning it's data object is either null
  ...
  /**
   * calculates the offsets of the chart to the border depending on the position
   * of an eventual legend or depending on the length of the y-axis and x-axis
  ...
  /**
   * calculates the required number of digits for the values that might be drawn
   * in the chart (if enabled)
   */
  ...
  /**
   * Generates an automatically prepared legend depending on the DataSets in the
   * chart and their colors.
   */
  ...
  protected void drawLegend() {
    ...
    switch (mLegend.getPosition()) {
      ...
      case BELOW_CHART_CENTER:
        ...
        Log.i(LOG_TAG, "content bottom: " + mContentRect.bottom + ", height: " + getHeight() + ", posY: " + posY
            + ", formSize: " + formSize);
   ...
   */
  protected float drawDescription() {
    float xoffset = Utils.convertDpToPixel(5f);
    mDrawCanvas.drawText(mDescription, xoffset, mOffsetBottom + 10, mDescPaint);
    yOffset= Utils.calcTextHeight(mDescPaint, mDescription);
    return yOffset;
  }
  ...
  /**
   * Returns true if there are values to highlight, false if there are no values
   * to highlight. Checks if the highlight array is null, has a length of zero
   * or if the first object is null.
   * 
   * @return
   */
   ...
   * programmatically highlight values. This DOES NOT generate a callback to the
  ...
  /**
   * Highlights the value at the given x-index in the given DataSet. Provide -1
   * as the x-index to undo all highlighting.
   * 
   * @param xIndex
   * @param dataSetIndex
   */
  ...
  protected void drawMarkers() {
    ...
    for (int i = 0; i < mIndicesToHightlight.length; i++) {
      ...
      if (xIndex <= mDeltaX && xIndex <= mDeltaX * mPhaseX) {
        ...
        // check bounds
        if (pos[0] < mOffsetLeft || pos[0] > getWidth() - mOffsetRight || pos[1] < mOffsetTop
            || pos[1] > getHeight() - mOffsetBottom)
          continue;
  ...
  /**
   * Returns the actual position in pixels of the MarkerView for the given Entry
  ...
  private float[] getMarkerPosition(Entry e, int dataSetIndex) {
    ...
    if (this instanceof CandleStickChart)
      ...
      PointF p = new PointF((float) (c.x + val * Math.cos(Math.toRadians(angle))), (float) (c.y + val
   ...
   * the specified animation time. If animate(...) is called, no further calling
   * of invalidate() is necessary to refresh the chart.
   * 
  ...
  /**
   * returns the average value for a specific DataSet (with a specific label) in
   * the chart
   * 
   * @param dataSetLabel
   * @return
  ...
  /**
   * Set this to true to enable logcat outputs for the chart. Default: disabled
   * 
   * @param enabled
   */
  ...
  /**
   * if set to true, units are drawn next to values in the chart, default: false
   * 
   * @param enabled
   */
  ...
  /**
   * Returns the Transformer class that contains all matrices and is responsible
   * for transforming values into pixels on the screen and backwards.
   * 
   * @return
   */
  ...
  /**
   * Set this to true to draw a user specified marker-view when tapping on chart
   * values (use the setMarkerView(MarkerView mv) method to specify a marker
   * view). Default: true
   * 
   * @param enabled
   */
  ...
  /**
   * returns the Entry object at the given index from the DataSet with the given
   * label.
   * 
   * @param index
   * @param dataSetLabel
  ...
  /**
   * Get all Entry objects at the given index across all DataSets. INFORMATION:
   * This method does calculations at runtime. Do not over-use in performance
   * critical situations.
   * 
   * @param xIndex
   * @return
  ...
  /**
   * Returns the ChartData object that ORIGINALLY has been set for the chart. It
   * contains all data in an unaltered state, before any filtering algorithms
   * have been applied.
   * 
   * @return
   */
  ...
  /**
   * Saves the current chart state with the given name to the given path on the
   * sdcard leaving the path empty "" will put the saved file directly on the SD
   * card chart is saved as a PNG image, example: saveToPath("myfilename",

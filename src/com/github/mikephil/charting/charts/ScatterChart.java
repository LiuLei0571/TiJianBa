...
public class ScatterChart extends BarLineChartBase<ScatterData> {
  ...
  protected void prepareContentRect() {
    if (isEmpty()) {
      ...
      mContentRect.set(mOffsetLeft - offset, mOffsetTop, getWidth() - mOffsetRight + offset, getHeight()
          - mOffsetBottom);
    }
  }
    ...
    for (int i = 0; i < mData.getDataSetCount(); i++) {
      ...
      for (int j = 0; j < valuePoints.length * mPhaseX; j += 2) {
        ...
        if (shape == ScatterShape.SQUARE) {
          ...
          mDrawCanvas.drawRect(valuePoints[j] - shapeHalf, valuePoints[j + 1] - shapeHalf, valuePoints[j] + shapeHalf,
              valuePoints[j + 1] + shapeHalf, mRenderPaint);
        ...
        for (int j = 0; j < positions.length * mPhaseX; j += 2) {
          ...
          if (mDrawUnitInChart) {
            ...
            mDrawCanvas.drawText(mValueFormatter.getFormattedValue(val) + mUnit, positions[j], positions[j + 1]
                - shapeSize, mValuePaint);
          } else {

            mDrawCanvas.drawText(mValueFormatter.getFormattedValue(val), positions[j], positions[j + 1] - shapeSize,
                mValuePaint);
          }
        }
      }
